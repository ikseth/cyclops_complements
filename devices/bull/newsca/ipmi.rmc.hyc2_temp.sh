...
_sensor_string="HYB_2 Temp"

_sensor_output=$( echo "${_sensor_ipmi_output}" | awk -F\| -v _test="$_sensor_string" '$1 ~ _test { _value = $2 * 100 / $8 ; _suma = _suma + _value ; _lines++ } END { _suma = _suma / _lines ; print _suma }' | cut -d'.' -f1 ) 
